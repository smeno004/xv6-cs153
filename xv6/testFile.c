...
int waitPid(void){
 ...
 printf(1, "\n  Step 2: testing waitpid(int pid, int* status, int options):\n");
  ...
  int i,j,k;
    ...
    printf(1, "\n  Step 3: testing the priority scheduler and setpriority(int priority)) systema call:\n");
    printf(1, "\n  Step 3: Assuming that the priorities range between range between 0 to 63\n");
    printf(1, "\n  Step 3: 0 is the highest priority. All processes have a default priority of 20\n");
    printf(1, "\n  Step 3: The parent processes will switch to priority 0\n");
    switchPriority(getpid(), 0);
    for (i = 0; i <  3; i++) {
	pid = fork();
	if (pid > 0 ) {
		continue;}
